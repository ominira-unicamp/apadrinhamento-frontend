...
const signupFormSchema = z
    .object({
        ...
        yearOfEntry: z
            .number({ required_error: "Ano de ingresso é obrigatório" })
            .int()
            .min(1900)
        ...
        picture: z
            ...
            .refine(
                (picture: any) =>
                    !picture?.length ||
                    ACCEPTED_IMAGE_TYPES.includes(picture[0]?.type),
                { message: "Selecione uma imagem válida" },
            )
            .refine(
                (picture: any) =>
                    !picture?.length || picture[0]?.size <= 5242880,
                { message: "Imagem muito grande. Limite: 5MB" },
            ),
    })
...
const editFormSchema = z.object({
    ...
    picture: z
        ...
        .refine(
            (picture: any) => !picture?.length || picture[0]?.size <= 5242880,
            { message: "Imagem muito grande. Limite: 5MB" },
        ),
});
...
export const SignupForm = () => {
    ...
    const {
        ...
        resolver: zodResolver(state?.edit ? editFormSchema : signupFormSchema) as any,
    ...
    const onSubmit = async (data: any) => {
        ...
        if (!state?.edit && !authCtx.token) {
            ...
            await toast.promise(
                UserService.update(
                    jwtDecode<{ id: string }>(authCtx.token).id,
                    profileData as any,
                ),
                {
                    success: {
    ...
    useEffect(() => {
        if (authCtx.status == true && !state?.edit) navigate("/dashboard");
            ...
            UserService.get(jwtDecode<{ id: string }>(authCtx.token).id).then(
                async (response) => {
                    ...
                    if (response.telephone) {
                        setValue("telephone", response.telephone, {
                            shouldValidate: false,
                            shouldDirty: false,
                        });
                    }
                    if (response.yearOfEntry) {
                        setValue("yearOfEntry", response.yearOfEntry, {
                            shouldValidate: false,
                            shouldDirty: false,
                        });
                    }
                    setValue(
                        "otherPronouns",
                        response.pronouns.filter(
                            (v) =>
                                !["Ela/Dela", "Ele/Dele", "Elu/Delu"].includes(
                                    v,
                                ),
                        )[0],
                    );
                    setValue(
                        "pronouns",
                        response.pronouns.filter((v) =>
                            ["Ela/Dela", "Ele/Dele", "Elu/Delu"].includes(v),
                        ),
                    );
                    setValue(
                        "otherEthnicity",
                        response.ethnicity.filter(
                            (v) =>
                                ![
                                    "Preta",
                                    "Branca",
                    ...
                    if (response.city && response.city !== "Cidade") {
                        try {
                            ...
                            const citySearchResponse = await axios.get(
                                `https://servicodados.ibge.gov.br/api/v1/localidades/municipios/${(response.city).replace(/ /g, '-').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()}`,
                            );

                            if (citySearchResponse.data && citySearchResponse.data.microrregiao) {
                                const stateId =
                                    citySearchResponse.data.microrregiao.mesorregiao.UF.id.toString();
                                ...
                                toast.warn(
                                    "Não foi possível encontrar a cidade salva nos dados do IBGE. Por favor, selecione seu estado e cidade novamente.",
                                );
                            }
                        } catch (error) {
    ...
    return (
        <div className="w-full min-h-screen bg-zinc-800 flex flex-col items-center p-5 gap-y-6 text-white">
            ...
            <h1 className="text-4xl text-center font-extrabold text-cyan-200">
                Bem-vinde ao Sistema de Apadrinhamento da Geologia
            </h1>
            ...
            >
                ...
                <TextField
                    ...
                    sx={inputStyle}
                    slotProps={{
                        inputLabel: { shrink: state?.edit && watch("name") },
                    }}
                    {...register("name")}
                />
                {errors.name && (
                ...
                <TextField
                    ...
                    value={
                        watch("telephone")
                            ? formatTelephone(watch("telephone") || "")
                            : ""
                    }
                    onChange={(e) => {
                ...
                {!submittedPic && (
                    <>
                        <div>
                            <p>
                                Se quiser, escolha uma foto para se apresentar:
                            </p>
                            <input
                                type="file"
                                multiple={false}
                                className="mt-2 file:bg-white file:rounded-lg file:text-black file:px-2"
                                accept="image/*"
                                {...register("picture")}
                            />
                        </div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Select
                        ...
                        onChange={(e) => {
                            ...
                            setValue("yearOfEntry", parseInt(value as string), {
                                shouldValidate: true,
                                shouldDirty: true,
                            });
                        }}
                    >
                        <MenuItem value="">
                            <em className="text-zinc-400">Selecione o ano</em>
                        </MenuItem>
                        {[
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 justify-evenly items-center">
                    ...
                    <TextField
                        ...
                        slotProps={{
                            inputLabel: {
                                shrink: state?.edit && watch("otherPronouns"),
                            },
                        }}
                        sx={inputStyle}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 justify-evenly items-center">
                    ...
                    <TextField
                        ...
                        slotProps={{
                            inputLabel: {
                                shrink: state?.edit && watch("otherEthnicity"),
                            },
                        }}
                        className="w-26 justify-self-center !mb-4"
                ...
                >
                    <MenuItem value="">
                        <em className="text-zinc-400">Selecione o estado</em>
                    </MenuItem>
                ...
                >
                    <MenuItem value="Cidade">
                        <em className="text-zinc-400">Cidade</em>
                    </MenuItem>
                    {cities.map((city) => (
                ...
                <p>
                    Você se idenfica como parte da comunidade LGBTQIA+? Se sim,
                    qual?
                </p>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 items-center">
                    ...
                    <TextField
                        ...
                        slotProps={{
                            inputLabel: {
                                shrink: state?.edit && watch("otherLgbt"),
                            },
                        }}
                        className="w-26 justify-self-center !mb-4"
                        {...register("otherLgbt")}
                ...
                <div>
                    <Slider
                        ...
                        onChange={(_, value) =>
                            setValue("parties", value as number)
                        }
                    />
                ...
                <TextField
                    ...
                    sx={inputStyle}
                    slotProps={{
                        inputLabel: { shrink: state?.edit && watch("hobby") },
                    }}
                    {...register("hobby")}
                />
                <TextField
                    ...
                    sx={inputStyle}
                    slotProps={{
                        inputLabel: { shrink: state?.edit && watch("music") },
                    }}
                    {...register("music")}
                />
                <TextField
                    ...
                    sx={inputStyle}
                    slotProps={{
                        inputLabel: { shrink: state?.edit && watch("games") },
                    }}
                    {...register("games")}
                />
                <TextField
                    ...
                    sx={inputStyle}
                    slotProps={{
                        inputLabel: { shrink: state?.edit && watch("sports") },
                    }}
                    {...register("sports")}
                />
